...
export class CompilerKeywordToken extends Token {
	...
	constructor(column: number, line: number, value: CompilerSymbols) {
...
export function scan(source: string, path: string): ScanResult {
	...
	const peek = (step = 0) => (pos < source.length ? source[pos + step] : null);
	...
	const handleTypeDecl = () => {
		...
		advance(); // ':'
		while (peek() && /\s/.test(checkNotNull(peek()))) advance();

		let type = "";
		while (
			peek() &&
			!/\s/.test(checkNotNull(peek())) &&
			peek() !== "%" &&
			peek() !== "\n"
		) {
			type += advance();
	...
	const scanNumber = () => {
		...
		if (peek() === "0") {
			...
			if (next === "x" || next === "X") {
				num += advance(); // 0
				num += advance(); // x
				while (peek() && /[0-9a-fA-F]/.test(checkNotNull(peek()))) {
					num += advance();
				}
			...
			if (next === "b" || next === "B") {
				num += advance(); // 0
				num += advance(); // b
				while (peek() && /[0-1]/.test(checkNotNull(peek()))) {
					num += advance();
				}
			...
			if (next === "o" || next === "O") {
				num += advance(); // 0
				num += advance(); // o
				while (peek() && /[0-7]/.test(checkNotNull(peek()))) {
					num += advance();
				}
		...
		// Standard Integer
		while (peek() && isDigit(checkNotNull(peek()))) {
			num += advance();
		}
		...
		if (peek() === "." && peek(1) && isDigit(checkNotNull(peek(1)))) {
			num += advance(); // consume '.'
			while (peek() && isDigit(checkNotNull(peek()))) {
				num += advance();
			}
	...
	const scanIdentifierOrKeyword = () => {
		const startCol = column;
		let word = "";
		while (peek() && isAlphaNumeric(checkNotNull(peek()))) word += advance();
	...
	const scanRegister = () => {
		...
		advance(); // '$'
		let name = "";
		while (peek() && isAlphaNumeric(checkNotNull(peek()))) name += advance();
		// if (peek() === " ") return scanNext();
		if (!name)
			error(
				`Expected register name after '$' but found "${peek()}" instead`,
				line,
				column,
			);
	...
	const scanCompilerKeyword = () => {
		...
		advance(); // '@'
		let kw = "";
		while (peek() && isAlphaNumeric(checkNotNull(peek()))) kw += advance();
		...
		tokens.push(
			new CompilerKeywordToken(startCol, line, kw as CompilerSymbols),
		);
	};
	...
	const scanMemberExpression = () => {
		...
		while (peek() === ".") {
			scanDelimiter();
			if (!peek() || !isAlphaNumeric(checkNotNull(peek()))) {
	...
	const scanNext = (): void => {
		...
		if (ch === "." && peek(1) === "." && peek(2) === ".") {
			...
			advance();
			advance();
			tokens.push(new Token(startCol, line, "DELIMITER", "..."));
			scanNext();
			return;
		}
		...
		if (BinOpValues.has(twoCharOp as BinOps)) {
			...
			tokens.push(
				new Token(
					startCol,
					line,
					BinOpValues.has(twoCharOp as BinOps) ? "BIN_OP" : "UNARY_OP",
					twoCharOp,
				),
			);
			scanNext();
		...
		if (
			BinOpValues.has(ch as BinOps) ||
			UnaryOpValues.has(ch as UnaryOps) ||
			...
			tokens.push(
				new Token(
					startCol,
					line,
					BinOpValues.has(op as BinOps)
						? "BIN_OP"
						: AssignOpValues.has(op as AssignOps)
							? "ASSIGN_OP"
							: "UNARY_OP",
					op,
				),
		...
		if (Object.values(Delimiters).includes(ch as typeof Delimiters[keyof typeof Delimiters])) {
			scanDelimiter();
			scanNext();
			return;
		}
